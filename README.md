<div id="top"></div>

<br />
<div align="center">

<!-- 🔰 BADGES -->
![GitHub repo size](https://img.shields.io/github/repo-size/codewith-salman/loopstudios-website)
![GitHub stars](https://img.shields.io/github/stars/codewith-salman/loopstudios-website?style=social)
![GitHub forks](https://img.shields.io/github/forks/codewith-salman/loopstudios-website?style=social)

[![Live Demo](https://img.shields.io/badge/Live%20Demo-View%20Project-brightgreen?style=for-the-badge&logo=vercel)](https://loop-studios-website-eta.vercel.app/)

[![GitHub Follow](https://img.shields.io/github/followers/codewith-salman?style=social)](https://github.com/codewith-salman)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Muhammad%20Salman%20Arshad-blue?style=social&logo=linkedin)](https://www.linkedin.com/in/muhammad-salmanarshad/)
[![Figma](https://img.shields.io/badge/Figma-codewithsalman-black?style=social&logo=figma)](https://www.figma.com/@codewithsalman)
[![Email](https://img.shields.io/badge/Email-msalmanwebdev%40gmail.com-red?style=social&logo=gmail)](mailto:msalmanwebdev@gmail.com)

<br />

<!-- 🔰 PROJECT LOGO -->
<img src="LoopStudios%20Landing%20Website/assets/images/logo.svg" alt="Loopstudios Website Logo" width="120"/>

<br />

<h1 align="center">🎮 Loopstudios – VR Studio Website</h1>

A sleek, immersive, and fully responsive **Landing Page Website UI** built using  
**HTML, CSS, and JavaScript**.  
Designed to showcase virtual reality creations with bold visuals, modern layouts, and strong storytelling.

<a href="https://loop-studios-website-eta.vercel.app/"><strong>➥ Live Demo</strong></a>

</div>

---

<!-- TABLE OF CONTENTS -->
<details>
  <summary>📑 Table of Contents</summary>
  <ol>
    <li><a href="#about-the-project">About The Project</a></li>
    <li><a href="#features">Features</a></li>
    <li><a href="#built-with">Built With</a></li>
    <li><a href="#contact">Contact</a></li>
  </ol>
</details>

---

## 📖 About The Project

<img src="LoopStudios%20Landing%20Website/assets/images/desktop-preview.jpg" alt="Loopstudios Website Desktop View" width="100%" />

The **Loopstudios Website** is a modern landing page UI concept inspired by creative and VR-focused studios.  
It emphasizes strong visual hierarchy, immersive imagery, and minimal yet powerful typography to deliver a futuristic and engaging user experience.

This project demonstrates your ability to build **marketing-focused landing pages**, manage responsive image-heavy layouts, and translate brand identity into clean UI design.  
From the hero section to interactive creation showcases, Loopstudios delivers a bold and professional digital presence.

Ideal for:
- Creative and VR studios  
- Tech startup landing pages  
- Product showcase websites  
- Practice with modern UI layouts  

---

## ✨ Features

- Fully responsive landing page layout  
- Bold hero section with immersive visuals  
- Interactive creations showcase  
- Clean grid-based design  
- Smooth hover effects and transitions  
- Modern typography and spacing 

---

## 🛠️ Built With

- **HTML5**  
- **CSS3**  
- **JavaScript (Vanilla)**  

---

## 📬 Contact

**Muhammad Salman Arshad**

- 💼 **LinkedIn:** [Muhammad Salman Arshad](https://www.linkedin.com/in/muhammad-salmanarshad/)  
- 🎨 **Figma:** [codewithsalman](https://www.figma.com/@codewithsalman)  
- 📧 **Email:** [msalmanwebdev@gmail.com](mailto:msalmanwebdev@gmail.com)

<p align="right">(<a href="#top">back to top</a>)</p>
